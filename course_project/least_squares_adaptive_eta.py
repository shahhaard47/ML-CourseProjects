...
def objective(labels, data, w):
	...
	for i in range(len(data)):
		if (labels.get(i) != None):
			obj += (labels[i] - dot(w, data[i]))**2;
	return obj

def leastSquaredAdaptive(dataFile, labelsFile):
	...
	## Gradient Descent
	# eta = 0.0001
	# eta_list = [10, 1, .1, .01, .001, .0001, .00001, .000001, .0000001, .00000001, .000000001, .0000000001, .00000000001]
...
if __name__ == '__main__':
	...
	predictions = leastSquaredAdaptive(dataFile, trainingLabels)
