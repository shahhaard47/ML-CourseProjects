...
# this should be returned with find_gini
def left_label(stump, colnum, data, trainlabels):
	lp = rp = float(0)
	numrows = len(data)
	for i in range(numrows):
		if (trainlabels.get(i) != None):
...
def find_stump(data, trainlabels, debug_print=False):
	...
	#find left side label (should be done in find_gini function by comparing lp>rp?0:1 but doing this rn)
	left = left_label(best_stump, best_k, data, trainlabels)

	return (best_k, best_stump, left)
